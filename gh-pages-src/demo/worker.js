import { AutoProcessor, Qwen3_5ForConditionalGeneration, TextStreamer, env } from './transformers.min.js?v=63'
...
const cacheBust = (async () => {
  try {
    ...
    for (const k of keys) {
      ...
      if (k.url.includes('model_q4f16.onnx') || k.url.includes('model_q4f16_q4f16.onnx')) await c.delete(k)
    }
  } catch(e) {}
})()
...
self.onmessage = async (e) => {
  ...
  if (type === 'generate') {
    ...
    try {
      ...
      await model.generate({ input_ids, attention_mask, ...genArgs })
