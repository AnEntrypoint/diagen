...
// Configuration
const GH_BASE = 'https://media.githubusercontent.com/media/AnEntrypoint/models/main/tts/';
const MODELS = {
    mimi_encoder: GH_BASE + 'mimi_encoder.onnx',
    text_conditioner: GH_BASE + 'text_conditioner.onnx',
    flow_lm_main: GH_BASE + 'flow_lm_main_int8.onnx',
    flow_lm_flow: GH_BASE + 'flow_lm_flow_int8.onnx',
    mimi_decoder: GH_BASE + 'mimi_decoder_int8.onnx',
    tokenizer: GH_BASE + 'tokenizer.model',
    voices: GH_BASE + 'voices.bin'
};
...
function splitIntoBestSentences(text) {
    ...
    for (const sentenceText of sentences) {
        ...
        const combined = currentChunk ? `${currentChunk} ${sentenceText}` : sentenceText;
        const combinedTokens = tokenizerProcessor.encodeIds(combined).length;

        if (currentChunk && combinedTokens > CHUNK_TARGET_TOKENS) {
            const currentTokens = tokenizerProcessor.encodeIds(currentChunk).length;
...
function decodeWavTo24k(buffer) {
    ...
    const numSamples = Math.floor((buffer.byteLength - 44) / 2 / channels);
    const mono = new Float32Array(numSamples);
    for (let i = 0; i < numSamples; i++) {
        mono[i] = view.getInt16(44 + i * channels * 2, true) / 32768;
    }
...
async function loadModels() {
    ...
    const CACHE_NAME = 'pockettts-models-v1';

    async function fetchCached(url, label) {
        const cache = await caches.open(CACHE_NAME);
        let response = await cache.match(url);
        if (!response) {
            postMessage({ type: 'status', status: `Downloading ${label}...`, state: 'loading' });
            response = await fetch(url);
            if (!response.ok) throw new Error(`Failed to fetch ${label}: ${response.statusText}`);
            await cache.put(url, response.clone());
        } else {
            postMessage({ type: 'status', status: `Loading ${label} (cached)...`, state: 'loading' });
        }
    ...
    try {
        ...
        // Load all models in parallel using Cache API for persistence
        const [encoderBuf, textCondBuf, flowMainBuf, flowFlowBuf, decoderBuf] = await Promise.all([
            fetchCached(MODELS.mimi_encoder, 'MIMI encoder'),
            fetchCached(MODELS.text_conditioner, 'text conditioner'),
            fetchCached(MODELS.flow_lm_main, 'flow LM main'),
            fetchCached(MODELS.flow_lm_flow, 'flow LM flow'),
            fetchCached(MODELS.mimi_decoder, 'MIMI decoder'),
        ]);

        postMessage({ type: 'status', status: 'Initializing models...', state: 'loading' });
        const [encoderRes, textCondRes, flowMainRes, flowFlowRes, decoderRes] = await Promise.all([
            ...
            ort.InferenceSession.create(textCondBuf, sessionOptions),
            ort.InferenceSession.create(flowMainBuf, sessionOptions),
            ort.InferenceSession.create(flowFlowBuf, sessionOptions),
        ...
        const tokenizerBuffer = await fetchCached(MODELS.tokenizer, 'tokenizer');
        ...
        const spModule = await import('./sentencepiece-js.mjs');
        ...
        // Load voices from voices/manifest.json
        postMessage({ type: 'status', status: 'Loading voices...', state: 'loading' });
        try {
            const manifestRes = await fetch('./voices/manifest.json');
            const wavFiles = manifestRes.ok ? await manifestRes.json() : ['cleetus.wav'];
            for (const filename of wavFiles) {
                const name = filename.replace(/\.wav$/i, '');
                try {
                    postMessage({ type: 'status', status: `Encoding voice: ${name}...`, state: 'loading' });
                    const buf = await fetchCached('./voices/' + filename, 'voice:' + name);
                    const audio = decodeWavTo24k(buf);
                    predefinedVoices[name] = await encodeVoiceAudio(audio);
                    console.log(`[TTS] Voice '${name}' encoded`);
                } catch(e) {
                    console.warn(`[TTS] Failed to encode voice '${name}':`, e.message);
                }
            }
        } catch(e) {
            console.error('[TTS] Voice manifest load failed:', e.message);
        }

        if (Object.keys(predefinedVoices).length > 0) {
            const first = Object.keys(predefinedVoices)[0];
            currentVoiceEmbedding = predefinedVoices[first];
            currentVoiceName = first;
        } else {
            console.warn('[TTS] No voices loaded');
            postMessage({ type: 'status', status: 'No voices available', state: 'loading' });
        }

        postMessage({ type: 'voices_loaded', voices: Object.keys(predefinedVoices), defaultVoice: currentVoiceName });
